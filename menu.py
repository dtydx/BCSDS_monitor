...
if __name__ == '__main__':
    ...
    print("6.获取mon 状态")
    print("7.获取ceph集群所有磁盘使用率")
    print("8.获取资源池状态")
    print("9.查看管理系统状态")
    print("10.获取当前节点磁盘状态")
    print("11.获取当前节点内存状态")
    print("12.获取当前系统负载")
    print("13.获取当前节点CPU状态")
    print("EXIT:输入q退出")
    inputs = ''
    while(inputs !='q'):
        inputs = input()
        ...
        if inputs == '8':  ##ceph集群所有磁盘使用率
            print(cat_ceph_status.get_pool_usage())
        if inputs == '9':  ##ceph集群所有磁盘使用率
            print(cat_ceph_status.get_crond_status())
        if inputs == '10':
